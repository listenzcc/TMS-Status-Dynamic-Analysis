...
# %%
files = sorted(DATA_DIR.rglob('*/results-word-detail-concat.json'))
print(files[:8])
...
fpath = DATA_DIR / f'err-results-20260920({query})-detail.csv'
df.to_csv(fpath)
...
if True:
    ...
    sns.boxenplot(df, x='Word', y='Real_Proportion', hue='tag', ax=ax)
    plt.title(query)
    plt.show()
