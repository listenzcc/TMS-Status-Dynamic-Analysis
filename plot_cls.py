...
# %%
methods = ['eLORETA', 'eLORETA-baseline', 'sLORETA', 'sLORETA-baseline']
method = methods[0]

parser = argparse.ArgumentParser()
parser.add_argument(
    '-m', '--method', help=f'Plot which method: {" | ".join(methods)}', default=method)
...
vertex_value_files = sorted(
    Path(f'./data/20484-{method}').glob('*.npy'))
logger.info(f'{len(vertex_value_files)=}')

# %%
OUTPUT_DIR = Path(f'./output-pre-post/{method}')
...
for pair in query_pairs:
    ...
    # Just make sure it plots only once
    if not stage == 'post':
        continue
